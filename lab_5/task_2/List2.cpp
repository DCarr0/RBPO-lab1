...
public:
    ...
    bool PopFront(T& element) final {
        if (head) {
            ...
            head = head->next;
            if (head) {
                head->prev = nullptr;  // Обнуляем связь с предыдущим узлом
            }
            delete temp;
            count--;
            return true;
    ...
    bool PopBack(T& element) final {
        if (tail) {
            ...
            if (tail) {
                ...
                head = nullptr;  // Если удаляем последний элемент, обнуляем head
            }
            delete temp;
            count--;
            return true;
...
DequeBasedOnUnidirectionalLinkedList<T>::DequeBasedOnUnidirectionalLinkedList(unsigned size)
    ...
    if (size > 0) {
        ...
        for (unsigned i = 1; i < size; ++i) {
            ...
            tail->next->prev = tail; // Устанавливаем связь с предыдущим узлом
...
int Main() {
    ...
    DequeBasedOnUnidirectionalLinkedList<int> deque3 = { 1, 2, 77, 4, 5 }; // Конструктор с использованием списка инициализации
