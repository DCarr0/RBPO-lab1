...
public:
    ...
    public:
        ...
        // Перегрузка оператора инкремента (++it)
        Iterator& operator++() {
            current = current->next;
            return *this;
        }

        // Перегрузка оператора разыменования (*it)
        T& operator*() const {
            return current->data;
        }

        // Перегрузка оператора неравенства (it != other)
    ...
    // Функции-члены
    unsigned  GetSize() const;  // Получить размер дека
    ...
    bool PushFront(const T& element);  // Добавить элемент в начало дека
    bool PopFront(T& element);         // Удалить элемент из начала дека
    bool PushBack(const T& element);   // Добавить элемент в конец дека
    bool PopBack(T& element);          // Удалить элемент из конца дека
    bool PeekFront(T& element);        // Посмотреть элемент в начале дека, не удаляя его
    bool PeekBack(T& element);         // Посмотреть элемент в конце дека, не удаляя его
...
template <typename T>
unsigned  DequeBasedOnUnidirectionalLinkedList<T>::GetSize() const {
    return count;
}
...
template <typename T>
bool DequeBasedOnUnidirectionalLinkedList<T>::PushFront(const T& element) {
...
template <typename T>
bool DequeBasedOnUnidirectionalLinkedList<T>::PushBack(const T& element) {
    Node<T>* newNode = new Node<T>(element);
    if (!head) {
        head = tail = newNode;
    }
...
template <typename T>
bool DequeBasedOnUnidirectionalLinkedList<T>::PopFront(T& element) {
    if (head) {
        element = head->data;
        Node<T>* temp = head;
        head = head->next;
...
template <typename T>
bool DequeBasedOnUnidirectionalLinkedList<T>::PopBack(T& element) {
    if (tail) {
        element = tail->data;
        if (head == tail) {
            delete tail;
...
template <typename T>
bool DequeBasedOnUnidirectionalLinkedList<T>::PeekFront(T& element) {
    if (head) {
        element = head->data;
        return true;
    }
...
template <typename T>
bool DequeBasedOnUnidirectionalLinkedList<T>::PeekBack(T& element) {
    if (tail) {
        element = tail->data;
        return true;
    }
...
int main() {
    // Пример использования класса DequeBasedOnUnidirectionalLinkedList
